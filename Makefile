...
# ============================================================
# KNN app settings (exported to included makefiles)
# ============================================================
KERNEL_NAME ?= kernel_gemm
KERNEL_SRC  ?= mm.cpp
HOST_SRC    ?= host.cpp

# Optional: define names for produced artifacts
EXE_NAME    ?= host
XCLBIN_NAME ?= kernel_gemm.xclbin

# Optional: dataset size override for emulation (host and kernel must match)
# Example usage:
#   make all TARGET=hw_emu NUM_PT_IN_SEARCHSPACE=512
NUM_PT_IN_SEARCHSPACE ?=

export KERNEL_NAME KERNEL_SRC HOST_SRC EXE_NAME XCLBIN_NAME NUM_PT_IN_SEARCHSPACE
...
help:
	...
	$(ECHO) ""
	$(ECHO) "  KNN variables (optional overrides):"
	$(ECHO) "      KERNEL_NAME=$(KERNEL_NAME)"
	$(ECHO) "      KERNEL_SRC=$(KERNEL_SRC)"
	$(ECHO) "      HOST_SRC=$(HOST_SRC)"
	$(ECHO) "      EXE_NAME=$(EXE_NAME)"
	$(ECHO) "      XCLBIN_NAME=$(XCLBIN_NAME)"
	$(ECHO) "      NUM_PT_IN_SEARCHSPACE=$(NUM_PT_IN_SEARCHSPACE) (empty means kernel default)"
	$(ECHO) ""
	$(ECHO) "  make clean "
