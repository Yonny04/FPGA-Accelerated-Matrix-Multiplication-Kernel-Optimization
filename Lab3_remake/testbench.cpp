...
//check if there are any errors between CPU and HLS 
float HLS_sum = checksum_C(C); 
//float CPU_sum = checksum_C(C_reference); 
float comparison = fabsf(HLS_sum - sum); 
...
//print_array_sum(C_reference);
// for loop to iterate through and cehck values

// Have a print sum -- salem

// Might want to compare directly sum or rounded sum -- salem
